...
export default function Projects() {
    ...
    const resetForm = () => setForm({ project_name: '', project_code: '', description: '', planned_start: '', planned_end: '', total_budget: '' });
        ...
        try {
            ...
            await Promise.all(projs.map(async (p) => {
                try {
                    ...
                    evmMap[p.id]  = computeEvm(taskRes.data || [], p.schedule_pct);
    ...
    const handleCreate = async (e) => {
        ...
        if (!trimmedName) { setError(t('projects.projectName') + ' ' + t('common.required') + '.'); return; }
        if (!trimmedCode) { setError(t('projects.projectCode') + ' ' + t('common.required') + '.'); return; }
        ...
        if (form.planned_end && form.planned_start && new Date(form.planned_end) < new Date(form.planned_start)) {
            setError(t('projects.endDate') + ' must be on or after ' + t('projects.startDate') + '.'); return;
        }
        setSaving(true);
        try {
            const res = await apiFetch('/projects', {
                method: 'POST',
                body: JSON.stringify({ ...form, project_name: trimmedName, project_code: trimmedCode, total_budget: parseFloat(form.total_budget) || 0 }),
            });
            if (!res.success) { setError(res.message || 'Failed to create project.'); return; }
        ...
        <div className="space-y-8">
            {successToast && (
                <div className="fixed top-6 right-6 z-50 bg-emerald-600 text-white px-5 py-3 rounded-xl shadow-lg shadow-emerald-200 flex items-center gap-2 text-sm font-semibold animate-in slide-in-from-top-2 fade-in duration-200">
                    <CheckCircle2 className="w-4 h-4" /> {t('projects.createdSuccess')}
                </div>
            )}

            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h2 className="text-3xl font-bold text-slate-800 tracking-tight">{t('projects.title')}</h2>
            ...
            {loading ? (
                <div className="flex items-center justify-center h-48 gap-3 text-slate-400">
                    <Loader2 className="w-6 h-6 animate-spin" /> {t('projects.loadingProjects')}
                </div>
            ) : projects.length > 0 ? (
                    ...
                    {projects.map((project) => {
                        ...
                        return (
                            <div key={project.id} onClick={() => setSelectedProject(project)}
                                ...
                                <p className="text-sm text-slate-500 mb-4 line-clamp-2 leading-relaxed">{project.description || t('common.noDescription')}</p>
                                {evm && (
                                    <div className="mb-4">
                                ...
                                <div className="mt-4 flex justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div className="text-emerald-500 flex items-center gap-1 text-xs font-semibold">
                                        {t('projects.openProject')} <ChevronRight className="w-4 h-4" />
                                    </div>
                                </div>
                            </div>
                        );
                ...
                <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setIsModalOpen(false)}>
                    <div role="dialog" className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl border border-slate-100 animate-in fade-in duration-200" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-bold text-slate-800">{t('projects.createProject')}</h3>
                        ...
                        {error && <div className="p-3 mb-4 rounded-lg bg-red-50/80 border border-red-100 text-red-600 text-xs text-center font-bold uppercase animate-pulse">{error}</div>}
                        <form onSubmit={handleCreate} className="space-y-4">
                            {[
                                { key: 'project_name', label: t('projects.projectName'), placeholder: 'e.g. Industrial Complex Phase 2', required: true },
                                { key: 'project_code', label: t('projects.projectCode'), placeholder: 'e.g. PRJ-2026-004', required: true },
                            ].map(field => (
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider ml-1">{t('projects.description')}</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider ml-1">{t('projects.startDate')}</label>
                                    <input type="date" required value={form.planned_start} onChange={e => setForm({ ...form, planned_start: e.target.value })}
                                        className="w-full px-4 py-3 bg-white/50 border border-slate-200 rounded-lg outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all text-slate-700 text-sm" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider ml-1">{t('projects.endDate')}</label>
                                    <input type="date" required min={form.planned_start || undefined} value={form.planned_end} onChange={e => setForm({ ...form, planned_end: e.target.value })}
                                        className="w-full px-4 py-3 bg-white/50 border border-slate-200 rounded-lg outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all text-slate-700 text-sm" />
                                </div>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider ml-1">{t('projects.totalBudget')}</label>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button type="submit" disabled={saving}
                                    ...
                                    {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> {t('projects.creating')}</> : t('projects.createProject')}
                                </button>
                            </div>
                        </form>
                    </div>

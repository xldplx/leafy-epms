...
const getAllPersonnel = async (req, res) => {
    try {
        let query = supabase.from('personnel').select('*, projects(project_name, project_code)');
...
const createPersonnel = async (req, res) => {
    const { employee_id, full_name, designation, zone, project_id, email, skill, level, position, status } = req.body;
...
const updatePersonnel = async (req, res) => {
    const updates = {};
    ['full_name','designation','zone','status','last_checkin','project_id','email','skill','level','position']
